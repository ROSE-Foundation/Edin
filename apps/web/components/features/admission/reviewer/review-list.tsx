...
export function ReviewList() {
  ...
  return (
    <div>
      <h1 className="mb-[var(--spacing-lg)] font-serif text-[28px] font-bold text-text-primary">
        My Reviews
      </h1>
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-[var(--spacing-xl)]">
          ...
          {completedReviews.length > 0 && (
            <section>
              <h2 className="mb-[var(--spacing-md)] font-sans text-[16px] font-semibold text-text-secondary">
...
function ReviewCard({
  ...
  return (
    <div className="rounded-[var(--radius-lg)] border border-surface-subtle bg-surface-raised p-[var(--spacing-md)] shadow-[var(--shadow-card)]">
      <h3 className="font-sans text-[16px] font-medium text-text-primary">
      ...
      <div className="mt-[var(--spacing-md)]">
        <button
          type="button"
          onClick={onReview}
          className={`rounded-[var(--radius-md)] px-[var(--spacing-md)] py-[var(--spacing-sm)] font-sans text-[14px] font-medium transition-[background-color,opacity] duration-[var(--transition-fast)] focus-visible:outline-2 focus-visible:outline-accent-primary focus-visible:outline-offset-2 ${
            completed
              ? 'border border-surface-subtle bg-surface-raised text-text-secondary hover:bg-surface-sunken'
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed top-1/2 left-1/2 z-40 max-h-[90vh] w-[90vw] max-w-[640px] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-[var(--radius-xl)] border border-surface-subtle bg-surface-raised p-[var(--spacing-lg)] shadow-[var(--shadow-modal)] focus:outline-none"
          aria-describedby={undefined}
        >
          <div className="mb-[var(--spacing-md)] flex items-start justify-between">
            <Dialog.Title className="font-serif text-[22px] font-bold text-text-primary">
              Application Review
            </Dialog.Title>
          ...
          {isLoading || !application ? (
            ...
            <>
              ...
              <div className="mt-[var(--spacing-xl)] border-t border-surface-subtle pt-[var(--spacing-lg)]">
                {hasSubmitted ? (
                  <div className="rounded-[var(--radius-md)] border border-surface-subtle bg-surface-sunken p-[var(--spacing-md)]">
