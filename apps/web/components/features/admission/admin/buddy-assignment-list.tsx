...
function getStatus(isActive: boolean, expiresAt: string): { label: string; className: string } {
  if (!isActive) {
    return { label: 'Inactive', className: 'text-brand-secondary bg-surface-sunken' };
  }
  if (new Date() > new Date(expiresAt)) {
    return { label: 'Completed', className: 'text-brand-accent bg-brand-accent/10' };
  }
  ...
  return (
    <div>
      <div className="mb-[var(--spacing-lg)] flex items-center justify-between">
        <h1 className="font-serif text-[28px] font-bold text-brand-primary">Buddy Assignments</h1>
        <select
          value={domainFilter}
                ...
                <span className="ml-auto w-[80px] text-right">
                  {assignment.isActive && (
                    <button
                      ...
                      className="rounded-[var(--radius-sm)] border border-surface-border px-[var(--spacing-sm)] py-[4px] font-sans text-[13px] text-brand-secondary transition-colors hover:bg-surface-sunken"
                    >
                      Override
                    </button>
                  )}
