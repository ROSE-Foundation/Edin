...
export function ApplicationConfirmation({
  ...
  if (status === 'DECLINED') {
    return (
      ...
      >
        <h2 className="font-serif text-[clamp(1.5rem,4vw,2rem)] leading-[1.2] font-bold text-text-primary">
          Application Update
        </h2>
        <p className="mt-[var(--spacing-lg)] font-sans text-[15px] leading-[1.6] text-text-secondary">
          After careful review, we are unable to move forward with your application at this time.
        </p>
        {declineReason && (
          <div className="mx-auto mt-[var(--spacing-md)] max-w-[480px] rounded-[var(--radius-md)] border border-surface-subtle bg-surface-sunken p-[var(--spacing-md)]">
            <p className="font-sans text-[14px] leading-[1.6] text-text-primary">{declineReason}</p>
          </div>
        )}
        <p className="mt-[var(--spacing-md)] font-sans text-[14px] leading-[1.5] text-text-secondary">
