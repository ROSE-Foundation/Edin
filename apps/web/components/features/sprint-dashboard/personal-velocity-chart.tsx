...
interface PersonalVelocityChartProps {
  data: VelocityEntry[];
}

export function PersonalVelocityChart({ data }: PersonalVelocityChartProps) {
    ...
    <section aria-label="Personal velocity">
      ...
      {showTable ? (
        ...
        <div
          ...
          role="img"
          aria-label={`Personal velocity: ${data.map((d) => `${d.sprintName} ${d.deliveredPoints} points`).join(', ')}`}
        >
