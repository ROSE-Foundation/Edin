...
function EventBadge({ action }: { action: string }) {
  let color = 'bg-text-secondary/10 text-text-secondary';
...
function DetailRow({ entry }: { entry: AuditLogEntry }) {
  return (
    <details className="mt-[var(--spacing-xs)]">
      <summary className="cursor-pointer text-xs text-accent-primary">View details</summary>
      <div className="mt-[var(--spacing-xs)] space-y-1 rounded-[var(--radius-sm)] bg-surface-base p-[var(--spacing-sm)] text-xs font-mono">
        {entry.previousState != null && (
          <div>
            <span className="font-semibold text-text-secondary">Previous:</span>{' '}
            <span className="text-text-primary">{JSON.stringify(entry.previousState)}</span>
          </div>
        )}
        {entry.newState != null && (
          <div>
            <span className="font-semibold text-text-secondary">New:</span>{' '}
            <span className="text-text-primary">{JSON.stringify(entry.newState)}</span>
          </div>
        )}
        {entry.reason && (
          <div>
        ...
        {entry.details != null && (
          <div>
            ...
            <span className="text-text-primary">{JSON.stringify(entry.details)}</span>
          </div>
        )}
        {entry.correlationId && (
          <div>
            <span className="font-semibold text-text-secondary">Correlation:</span>{' '}
            <span className="text-text-primary">{entry.correlationId}</span>
          </div>
        )}
      </div>
    </details>
              ...
              <tbody>
                {logs.map((entry) => (
                  <tr key={entry.id} className="border-b border-surface-subtle last:border-b-0">
                    ...
                    <td className="px-[var(--spacing-sm)] py-[var(--spacing-sm)] text-text-primary">
