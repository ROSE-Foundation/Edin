...
const DOMAIN_STYLES: Record<string, { bg: string; border: string; badge: string }> = {
  ...
  Finance: {
    ...
    badge: 'bg-domain-finance text-text-primary',
  },
  Impact: {
    bg: 'bg-[#B06B6B]/5',
    border: 'border-[#B06B6B]/20',
    badge: 'bg-domain-impact text-text-primary',
  },
  Governance: {
    bg: 'bg-[#7B6B8A]/5',
...
export function WorkingGroupCard({ group, isMember, onJoin, isJoining }: WorkingGroupCardProps) {
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-[var(--spacing-md)] font-serif text-[18px] font-bold leading-[1.3] text-text-primary">
        <Link href={`/dashboard/working-groups/${group.id}`} className="hover:underline">
          {group.name}
        </Link>
      </h3>

      <p className="mt-[var(--spacing-sm)] flex-1 font-serif text-[14px] leading-[1.65] text-text-secondary">
        {group.description}
      </p>
      ...
      <div className="mt-[var(--spacing-lg)]">
        {isMember ? (
          <Link
            ...
            className="inline-flex min-h-[44px] w-full items-center justify-center rounded-[var(--radius-md)] border border-surface-subtle bg-surface-sunken px-[var(--spacing-md)] font-sans text-[14px] font-medium text-text-secondary transition-colors duration-[var(--transition-fast)] hover:bg-surface-raised"
          ...
          <button
            ...
            disabled={isJoining}
            className="inline-flex min-h-[44px] w-full items-center justify-center rounded-[var(--radius-md)] bg-accent-primary px-[var(--spacing-md)] font-sans text-[14px] font-medium text-surface-raised transition-opacity duration-[var(--transition-fast)] hover:opacity-90 disabled:opacity-50"
