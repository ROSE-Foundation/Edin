    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed right-0 top-0 z-50 flex h-full w-full max-w-[640px] flex-col bg-surface-raised shadow-[var(--shadow-modal)]">
          <div className="flex items-center justify-between border-b border-surface-subtle px-[var(--spacing-lg)] py-[var(--spacing-md)]">
            <Dialog.Title className="font-serif text-[20px] font-bold text-text-primary">
              Add Repository
            </Dialog.Title>
            ...
            <Dialog.Close asChild>
              <button
                type="button"
                className="rounded-[var(--radius-sm)] p-[var(--spacing-xs)] text-text-secondary transition-colors hover:bg-surface-sunken"
                aria-label="Close"
              >
                <CloseIcon />
              </button>
          ...
          >
            <div className="flex flex-col gap-[var(--spacing-xs)]">
              <label
                htmlFor="fullName"
                className="font-sans text-[14px] font-medium text-text-primary"
              >
                Repository
              </label>
              <input
                ...
                {...register('fullName')}
                className="rounded-[var(--radius-md)] border border-surface-subtle-input bg-surface-base px-[var(--spacing-sm)] py-[var(--spacing-sm)] font-sans text-[15px] text-text-primary outline-none transition-[border-color] duration-[var(--transition-fast)] placeholder:text-text-secondary/50 focus:border-accent-primary focus:ring-2 focus:ring-accent-primary/20"
              />
              {errors.fullName && (
            ...
            <div className="mt-auto flex justify-end gap-[var(--spacing-sm)] border-t border-surface-subtle pt-[var(--spacing-md)]">
              ...
              <button
                ...
                className="rounded-[var(--radius-md)] bg-accent-primary px-[var(--spacing-md)] py-[var(--spacing-sm)] font-sans text-[14px] font-medium text-white transition-[background-color] duration-[var(--transition-fast)] hover:bg-accent-primary/90 disabled:opacity-50"
              >
