...
export function AgreementRateCard({ rates, isLoading }: AgreementRateCardProps) {
  if (isLoading) {
    return (
      <div className="animate-pulse rounded-[12px] border border-surface-subtle bg-surface-raised p-[var(--spacing-lg)]">
  ...
  if (!rates || rates.overall.totalReviewed === 0) {
    return (
      <div className="rounded-[12px] border border-surface-subtle bg-surface-raised p-[var(--spacing-lg)] shadow-[var(--shadow-card)]">
        <h3 className="font-sans text-[14px] font-medium text-text-primary">AI-Human Alignment</h3>
  ...
  return (
    <div className="rounded-[12px] border border-surface-subtle bg-surface-raised p-[var(--spacing-lg)] shadow-[var(--shadow-card)]">
      ...
      <div className="mt-[var(--spacing-xs)] font-sans text-[12px] text-text-secondary">
      ...
      {byDomain.length > 0 && (
        <div className="mt-[var(--spacing-md)] flex flex-col gap-[var(--spacing-xs)]">
          <h4 className="font-sans text-[12px] font-medium uppercase tracking-wider text-text-secondary">
            By Domain
          </h4>
          {byDomain.map((d) => {
            ...
            return (
              <div key={d.domain} className="flex items-center gap-[var(--spacing-sm)]">
                <span className="w-[90px] font-sans text-[12px] text-text-secondary">
                ...
                <span className="w-[40px] text-right font-sans text-[12px] text-text-primary">
                  {d.agreementRate}%
                </span>
              </div>
            );
